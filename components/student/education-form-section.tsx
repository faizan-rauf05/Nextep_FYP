...
export function EducationFormSection() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      // normalize education as an array
      let eduArray: Education[] = [];
      if (parsedUser.education) {
        if (Array.isArray(parsedUser.education)) {
          eduArray = parsedUser.education;
        } else {
          const eduObj = parsedUser.education as {
            degree?: string;
            major?: string;
            university?: string;
          ...
          eduArray = [
            {
              ...
              school: parsedUser.education.university || "",
              degree: parsedUser.education.degree || "",
              field: parsedUser.education.major || "",
              startYear: parsedUser.education.startYear?.toString() || "",
              endYear: parsedUser.education.endYear?.toString() || "",
            },
          ];
        }
      }
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      const updatedEducations = editEducations;
      setEducations(updatedEducations);

      if (user) {
        // Update user object in localStorage
        const updatedUser = { ...user, education: updatedEducations };
        setUser(updatedUser);
