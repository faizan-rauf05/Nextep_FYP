...
export default function MySessionsPage() {
  ...
  // Read studentId from localStorage
  useEffect(() => {
    const storedUser = localStorage.getItem("user");
    if (storedUser) {
      const userData = JSON.parse(storedUser);
      // Use id or _id depending on what your backend returns
      console.log(userData.id);
      setStudentId(userData.id);
    }
  }, []);

  // Fetch meetings when studentId is available
  useEffect(() => {
    if (!studentId) return; // wait until we have the studentId
